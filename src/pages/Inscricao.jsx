...
export default function Inscricao() {
  ...
  function validate() {
    ...
    const e = {}
    if (!form.nomeDiscord.trim()) e.nomeDiscord = req
    if (!BATTLETAG_REGEX.test(form.battletag)) e.battletag = t('form.errors.battletag')
    if (!form.pais) e.pais = req
    if (form.linguas.length === 0) e.linguas = t('form.errors.linguas')
    if (!form.elo) e.elo = req
    if (!form.rolePrimaria) e.rolePrimaria = req
    if (!form.roleSecundaria) e.roleSecundaria = req
    if (!form.querCapitao) e.querCapitao = req
    if (!form.titularReserva) e.titularReserva = req
    if (!form.aceitouRegras) e.aceitouRegras = t('form.errors.regras')
    return e
  }
    ...
    return (
      <main className="page">
        <div className="inscricao-success">
          ...
          <h2>{t('form.already_registered')}</h2>
  ...
  if (!inscricaoAberta) {
    return (
      <main className="page">
        <div className="inscricao-success">
          <div className="inscricao-success-icon">🔒</div>
          <h2 style={{ color: 'var(--text)' }}>{t('form.closed')}</h2>
          <p style={{ maxWidth: 420, textAlign: 'center', fontFamily: "'Barlow', sans-serif", fontSize: 14, color: 'var(--text2)', lineHeight: 1.6, marginTop: 8 }}>
            {conteudo.proximoEvento
              ? `${t('form.closed_hint').split('.')[0]}. ${t('home.next_event')}: ${conteudo.proximoEvento}.`
              : t('form.closed_hint')
            }
          </p>
        </div>
  ...
  return (
    <main className="page">
      ...
      <form className="inscricao-form" onSubmit={handleSubmit} noValidate>
        ...
        {/* Titular / Reserva */}
        <div className="form-group">
          <label className="form-label">{t('form.fields.titular_reserva')} *</label>
          <div className="form-toggle-group">
            {TITULAR_OPTS.map((o) => (
              <button
                key={o}
              ...
              >
                {t(`form.fields.${o.toLowerCase()}`)}
              </button>
            ))}
          </div>
          <div style={{ fontSize: '12px', color: 'var(--text2)', marginTop: '6px' }}>
            {t('form.fields.titular_hint')}
          </div>
        ...
        <div className="form-group">
          ...
          <div className="form-toggle-group">
            {CAPITAO_OPTS.map((o) => (
              ...
              >
                {t(`form.fields.cap_${o.toLowerCase()}`)}
              </button>
            ))}
          </div>
        ...
        <div className="form-group">
          <label className="form-checkbox">
            ...
            />
            <span>
              <Trans i18nKey="form.fields.regras">
                Li e aceito as <Link to="/regras" target="_blank" style={{ color: 'var(--blue)', textDecoration: 'underline' }}>regras da Copa Inhouse</Link>
              </Trans>
            </span>
          </label>
